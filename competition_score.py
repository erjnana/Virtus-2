...
# FUNÇÃO DE PONTUAÇÃO
# =======================
def compute_competition_score(pv, cp_max, N_horizontal=Nhor_DEFAULT, NR=NR_DEFAULT):
    """
    ...
    if cp_min > cp_allowed:
        print(f"\n⚠️ Atenção: não é possível atingir EE=1.5 sem ultrapassar 20kg. Usando carga mínima = 0.")
        cp_min = 0.0
    ...
    cp_values = []
    PVOO_values = []
    step = (cp_allowed - cp_min) / 5 if cp_allowed > cp_min else 1.0  # divide em 10 passos
    cp = cp_min
